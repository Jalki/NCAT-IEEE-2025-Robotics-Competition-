...
//   delay(3000);
   ...
   moveForward(650);// forward to go outside cave and stop by the right of the Neb container
   delay(4500);
     moveLeft(241);
     delay(3000);
     moveBackward(101);
...
void waitForSwitch()
{
  ...
  while(!readyToRun)
  {
    ...
    int lightDelay = 50;
    if(digitalRead(SW2) == LOW && analogRead(A0) < 200)
    {
      while(analogRead(A0) < 200 && !readyToRun)
      {
        lightDelay --;
        delay(1);
        if(lightDelay <= 0)
...
void checkTime(unsigned long startTime)
{
  unsigned long currentTime = millis();
  if(currentTime >= startTime + (3 * 1000 * 60))
  {
    robotDone();
  }
}
